...
use crate::event::{Events, WaitUntil};
use crate::options::*;
use crate::rx::*;
    ...
    pub fn call_send(&mut self, input_data: Vec<u8>) -> Result<(), Error> {
        ...
        match self.connection_state {
            ...
            ConnectionState::SynSent | ConnectionState::SynReceived => {
                ...
                self.events.return_continue_after(WaitUntil::Acknowledged {
        ...
        match self.connection_state {
            ...
            | ConnectionState::Established { .. } => {
                ...
                if seg.flags.contains(SegmentFlags::ACK) {
                    ...
                    {
                        if tx_state == ChannelState::Fin && seg.ackn == self.tx.next {
